...
public class ChangePassControl extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        DAO dao = new DAO();
        String olPassDAO = dao.getUserByID(id).getPassword();
        
        if (!oldPass.equalsIgnoreCase(olPassDAO)){
            request.setAttribute("mess", "Old Password isn't corectly!");
        } else {
            if (!newPass.equalsIgnoreCase(reNewPass)){
                request.setAttribute("mess", "New Passwords don't match!");
            } else {
